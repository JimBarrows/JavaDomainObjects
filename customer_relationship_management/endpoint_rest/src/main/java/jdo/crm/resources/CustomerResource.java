...
public class CustomerResource {
	...
	@ApiOperation(value = "Create CustomerResource", notes = "Create a customer from the customer dto.", response = CustomerDto.class)
	public CustomerDto create(final CustomerDto customer) {
		...
		Customer customerEntity = new Customer(party);
		customerEntity.setRelationshipTo(customerRelationship);
	...
	@Consumes(APPLICATION_JSON)
	@Produces(APPLICATION_JSON)
	@ApiOperation(value = "Update CustomerResource", notes = "Updates a customer, using the id in the path, and customer data.", response = CustomerDto.class)
	@Transactional
	 ...
	 * List all customers.
	 *
	 * @param offset An optional offset into the returning array of customers.
	...
	public List<CustomerDto> listAll(@QueryParam("offset") final Integer offset,
		...
		partyRepo.findBy(hasActiveCustomerRelationshipWith(configuration.company()), Optional.ofNullable(offset),
				Optional.ofNullable(limit)).forEach(party -> customerDtoList.add(new CustomerDto(new Customer(party))));
	...
	public CustomerDto findById(@PathParam("id") final UUID id) {
		...
		return new CustomerDto(new Customer(party));
	}
}
